...
export default function ClinicAdmin() {
    ...
    const { doctors, isLoading, fetchDoctors, deleteDoctor, setActiveTab } = useClinicAdminPanelStore();
            ...
            <div className="w-full">
                <div className="flex justify-between items-center">
                    ...
                    <div className="flex gap-2 items-center">
                        ...
                        <Button variant="destructive" onClick={() => { handleDeleteDoctor(id); toast.dismiss(t.id) }}>Delete</Button>
                    </div>
                </div>
            </div>
        ))
    ...
    const filteredDoctors = currentTab === "All"
        ? doctors
    ...
    return (
        <div className="p-6 space-y-8 w-full">
                            ...
                            <div className="flex overflow-x-auto">
                                ...
                                <TabsList className="bg-white  rounded-none">
                                    {specializations.map((spec) => (
                                        <TabsTrigger key={spec} value={spec} className="font-bold" > <span className={`${currentTab === spec && "text-primary border-b-4 border-primary"}`}>{spec} </span></TabsTrigger>
                                    ))}
                                </TabsList>
                            </div>
                        </div>
                        <div className="my-4">
                            <span className="font-bold"> {filteredDoctors?.length} </span> in total
                        </div>
                        {specializations.map((spec) => (
                                                ...
                                                <TableRow key={doctor._id}>
                                                    ...
                                                    <TableCell>
                                                        <div className="flex gap-2">
                                                            ...
                                                            <Eye className="h-4 w-4 text-primary hover:cursor-pointer" onClick={() => handleViewDoctor(doctor._id)} />


                                                            <Edit className="h-4 w-4 text-green-500 hover:cursor-pointer" onClick={() => handleEditDoctor(doctor._id)} />


                                                            <Trash className="h-4 w-4 text-red-500 hover:cursor-pointer" onClick={() => handleDeleteDoctorAlert(doctor._id)} />
            ...
            <EditDoctor isDrawerOpen={isEditDrawerOpen} setIsDrawerOpen={handleEditDrawerOpen} oldDoctor={doctors?.find(doctor => doctor._id === activeDoctorId)} />
